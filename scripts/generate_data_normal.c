...
/* ==== New runtime mode toggles ==== */
#define USE_NORMAL_RUNTIME 1          /* 1 = use Normal runtime, 0 = original */
#define NORMAL_MEAN_HOURS 1.5         /* target mean around 1–2 hours */
#define NORMAL_SIGMA_HOURS 0.740      /* gives ~50% mass between 1h and 2h */
#define CALIBRATION_SAMPLES 200000    /* MC samples to match expected compute */
#define NORMAL_TAIL_PROB 0.18         /* probability of drawing from heavy tail */
#define NORMAL_TAIL_MEAN_HOURS 16.0   /* mean (pre-scale) of the tail component */
#define NORMAL_TAIL_LOG_SIGMA 0.8     /* lognormal dispersion for tail draws */
#define NORMAL_RARE_THRESHOLD_H 48.0  /* hours; jobs above this are rare */
#define NORMAL_RARE_KEEP_PROB 0.03    /* acceptance probability for >48h draws */
#define NORMAL_MAX_HOURS 96.0         /* absolute cap for Normal-mode runtimes */
...
#define UMED              4.5
#define UHI               8         /* SYSTEM SIZE is 2^UHI == 128          */
#define UPROB             0.86
...
/* Globals for scaling to preserve expected daily compute */
static double RUNTIME_SCALE = 1.0;

/* New helpers */
static double randn_box_muller(void);
static double truncnorm_hours(double mu_h, double sigma_h);
static double sample_tail_lognormal_hours(void);
static double sample_runtime_hours_unscaled(void);
static double sample_normal_mode_seconds_unscaled(void);
static unsigned long time_from_nodes_normal(unsigned int nodes);
static double calibrate_runtime_scale_single_type(
    double SerialProb, double Pow2Prob, double ULow, double UMed, double UHi, double Uprob,
    double a1, double b1, double a2, double b2, double pa, double pb, int samples);
...
int main(int argc, char **argv)
{
  ...
  /* for each job, calculate its type (if needed) , arrival time
   ...
   */
  fprintf(outfile, "; Version: 2\n");
  fprintf(outfile, "; Acknowledge: Uri Lublin, Hebrew University\n");
  fprintf(outfile, "; Information: http://www.cs.huji.ac.il/labs/parallel/workload\n");
...
#if USE_NORMAL_RUNTIME
  /* Calibrate scale to preserve E[nodes * runtime] */
  RUNTIME_SCALE = calibrate_runtime_scale_single_type(
      SerialProb[ACTIVE], Pow2Prob[ACTIVE], ULow[ACTIVE], UMed[ACTIVE], UHi[ACTIVE], Uprob[ACTIVE],
      a1[ACTIVE], b1[ACTIVE], a2[ACTIVE], b2[ACTIVE], pa[ACTIVE], pb[ACTIVE],
      CALIBRATION_SAMPLES);
#endif
...
{
#if USE_NORMAL_RUNTIME
  (void)alpha1; (void)beta1; (void)alpha2; (void)beta2; (void)pa; (void)pb;
  return time_from_nodes_normal(nodes);
#else
  double hg;
...
/* Truncated Normal in hours (>0), simple rejection */
static double truncnorm_hours(double mu_h, double sigma_h)
{
  double x_h;
  do {
    x_h = mu_h + sigma_h * randn_box_muller();
  } while (x_h <= 0.0);
  return x_h;
}

/* Heavy-tail component: lognormal with throttled >48h acceptance */
static double sample_tail_lognormal_hours(void)
{
  const double sigma = NORMAL_TAIL_LOG_SIGMA;
  const double mu = log(NORMAL_TAIL_MEAN_HOURS) - 0.5 * sigma * sigma;
  for (int attempt = 0; attempt < 128; ++attempt) {
    double hours = exp(mu + sigma * randn_box_muller());
    if (hours <= 0.0)
      continue;
    if (hours > NORMAL_MAX_HOURS)
      hours = NORMAL_MAX_HOURS;
    if (hours <= NORMAL_RARE_THRESHOLD_H || drand48() < NORMAL_RARE_KEEP_PROB || attempt >= 64)
      return hours;
  }
  return NORMAL_RARE_THRESHOLD_H; /* fallback */
}

/* Sample a runtime in hours before global scaling */
static double sample_runtime_hours_unscaled(void)
{
  double u = drand48();
  if (u < NORMAL_TAIL_PROB) {
    return sample_tail_lognormal_hours();
  }
  return truncnorm_hours(NORMAL_MEAN_HOURS, NORMAL_SIGMA_HOURS);
}

static double sample_normal_mode_seconds_unscaled(void)
{
  double hours = sample_runtime_hours_unscaled();
  if (hours > NORMAL_MAX_HOURS)
    hours = NORMAL_MAX_HOURS;
  if (hours < (1.0 / 3600.0))
    hours = (1.0 / 3600.0);
...
/* Normal runtime (in seconds), scaled to preserve expected compute */
static unsigned long time_from_nodes_normal(unsigned int nodes)
{
  (void)nodes; /* runtime independent of nodes in this mode */
  const unsigned long cap_seconds = (unsigned long)llround(NORMAL_MAX_HOURS * 3600.0);
  for (int attempt = 0; attempt < 256; ++attempt) {
    double seconds = sample_normal_mode_seconds_unscaled() * RUNTIME_SCALE;
    if (seconds < 1.0)
      seconds = 1.0;
    unsigned long sec = (unsigned long)llround(seconds);
    if (sec <= cap_seconds)
      return sec;
  }
  return cap_seconds;
...
/* Compute scale s so that E[N * R_new] == E[N * R_old] in expectation.
   We estimate both expectations via Monte Carlo. */
static double calibrate_runtime_scale_single_type(
    double SerialProb, double Pow2Prob, double ULow, double UMed, double UHi, double Uprob,
    double a1, double b1, double a2, double b2, double pa, double pb, int samples)
{
  unsigned short saved_state[3];
  unsigned short seed_buf[3] = { 0x330E, 0xABCD, 0x1234 };
  unsigned short *prev = seed48(seed_buf);
  saved_state[0] = prev[0];
  saved_state[1] = prev[1];
  saved_state[2] = prev[2];

  double sum_nodes_rt_old = 0.0;
  double sum_nodes_rt_new = 0.0;

  for (int i = 0; i < samples; ++i) {
    unsigned int nodes = calc_number_of_nodes(SerialProb, Pow2Prob, ULow, UMed, UHi, Uprob);
    ...
    double hg;
    do {
      hg = hyper_gamma(a1, b1, a2, b2, p);
    } while (hg > TOO_MUCH_TIME);
    unsigned long rt_sec_old = (unsigned long)llround(exp(hg));

    double rt_sec_new = sample_normal_mode_seconds_unscaled();

    sum_nodes_rt_old += ((double)nodes * (double)rt_sec_old);
    sum_nodes_rt_new += ((double)nodes * rt_sec_new);
  }

  seed48(saved_state);

  double s = sum_nodes_rt_old / sum_nodes_rt_new;
  if (s <= 0.0 || !isfinite(s))
    s = 1.0;
  return s;
}
